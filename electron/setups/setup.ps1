...
# STEP 2.5: Unduh main.js dan letakkan ke folder aplikasi
$mainJsUrl = "https://github.com/zenzalepik/rilis_ep/raw/main/main.js"
$mainJsTarget = "$base\dist\win-unpacked\resources\app\electron\main.js"
$tempMainJs = "$env:TEMP\main.js"

Write-Host "⬇️ Mengunduh main.js..."
Invoke-WebRequest -Uri $mainJsUrl -OutFile $tempMainJs -UseBasicParsing

# Pastikan direktori tujuan ada
$mainJsDir = Split-Path $mainJsTarget
New-Item -ItemType Directory -Force -Path $mainJsDir | Out-Null

# Hapus main.js lama jika ada
if (Test-Path $mainJsTarget) {
    Write-Host "🧹 Menghapus main.js lama..."
    Remove-Item $mainJsTarget -Force
}

Write-Host "📦 Menyalin main.js ke folder aplikasi..."
Copy-Item -Path $tempMainJs -Destination $mainJsTarget -Force

# Bersih-bersih file sementara
Remove-Item $tempMainJs -Force
Write-Host "✅ main.js berhasil diperbarui."
