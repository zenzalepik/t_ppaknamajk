...
import { tableDataRiwayatTransaksiMember } from './tableDataRiwayatTransaksiMember';
...
const titleSection = 'Riwayat Transaksi Member';

export default function RiwayatTransaksiMember({ onBack }) {
  const urlExport = '/riwayat_transaksi_member/';
  ...
  const {
    data: laporanRiwayatTransaksiMember,
    error,
    isLoading,
  } = useQuery({
    queryKey: ['laporanRiwayatTransaksiMember', currentPage, searchText],
    queryFn: () =>
  ...
  const rows =
    laporanRiwayatTransaksiMember?.data?.length > 0
      ? laporanRiwayatTransaksiMember.data
          // .filter((row) => row.jenisTransaksi === 'Member')
          .map((row, index) => {
      ...
      /> */}
        {/* {searchText == '' ? (
          ...
          <div className="relative">
            ...
            <EvoTable
              id="tableToPrint"
              tableData={tableDataRiwayatTransaksiMember}
              currentPage={currentPage}
              totalPages={laporanRiwayatTransaksiMember?.totalPages}
