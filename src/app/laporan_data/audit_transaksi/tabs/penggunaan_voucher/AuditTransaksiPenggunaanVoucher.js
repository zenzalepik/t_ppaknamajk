...
import FilterLapAudTranPenggunaanVoucher from './FilterLapAudTranPenggunaanVoucher';
...
export default function AuditTransaksiPenggunaanVoucher() {
  ...
  const rows = tableDataAuditTransaksiPenggunaanVoucher.rows.map((row) => ({
    ...row,
    // fileSettlementName: (
    //   <a
    ...
    <EvoCardSection className="!p-0 !bg-transparent !shadow-none">
      ...
      <EvoTable
        ...
        onPageChange={
          ...
          // columns={tableDataAuditTransaksiPenggunaanVoucher.columns} rows={rows}
        }
        rows={rows}
      />
    </EvoCardSection>
