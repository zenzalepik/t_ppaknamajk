...
import EditManlessForm from './forms/EditManlessForm';
...
import { fetchApiPengaturanParameterManless } from './api/fetchApiPengaturanParameterManless';
import { fetchApiPengaturanParameterManlessDelete } from './api/fetchApiPengaturanParameterManlessDelete';
...
const titleSection = 'Parameter Manless';

export default function ParameterManlessSection({ onBack }) {
  ...
  const {
    data: pengaturanParameterManless,
    error,
    isLoading,
  } = useQuery({
    queryKey: ['pengaturanParameterManless', currentPage],
    queryFn: () =>
      fetchApiPengaturanParameterManless({
  ...
  const handleEdit = (id) => {
    ...
    const dataDipilih = pengaturanParameterManless?.data?.find(
      (item) => item.id === id
    );
    if (dataDipilih) {
      setSelectedData({
        id: dataDipilih.id,
        nama: dataDipilih.tipe_manless || '',
  ...
  const handleDelete = async (id) => {
    ...
    await fetchApiPengaturanParameterManlessDelete(
      id,
      setNotifMessage,
      setNotifType
    );
    ...
    queryClient.invalidateQueries(['pengaturanParameterManless']);
    // logic delete
  };
  ...
  const dataApi = pengaturanParameterManless || {};

  const rows =
    pengaturanParameterManless?.data?.length > 0
      ? pengaturanParameterManless.data.map((row, index) => ({
          no: index + 1,
          nama: <b>{row.tipe_manless}</b>,
