...
import EditKendaraanForm from './forms/EditKendaraanForm';
...
import { fetchApiPengaturanParameterKendaraan } from './api/fetchApiPengaturanParameterKendaraan';
import { fetchApiPengaturanParameterKendaraanDelete } from './api/fetchApiPengaturanParameterKendaraanDelete';
...
const titleSection = 'Parameter Kendaraan';

export default function ParameterKendaraanSection({ onBack }) {
  ...
  const {
    data: pengaturanParameterKendaraan,
    error,
    isLoading,
  } = useQuery({
    queryKey: ['pengaturanParameterKendaraan', currentPage],
    queryFn: () =>
      fetchApiPengaturanParameterKendaraan({
  ...
  const handleEdit = (id) => {
    ...
    const dataDipilih = pengaturanParameterKendaraan?.data?.find(
      (item) => item.id === id
    );
    if (dataDipilih) {
      setSelectedData({
        id: dataDipilih.id,
        nama: dataDipilih.tipe_kendaraan || '',
  ...
  const handleDelete = async (id) => {
    ...
    await fetchApiPengaturanParameterKendaraanDelete(
      id,
      setNotifMessage,
      setNotifType
    );
    ...
    queryClient.invalidateQueries(['pengaturanParameterKendaraan']);
    // logic delete
  };
  ...
  const dataApi = pengaturanParameterKendaraan || {};

  const rows =
    pengaturanParameterKendaraan?.data?.length > 0
      ? pengaturanParameterKendaraan.data.map((row, index) => ({
          no: index + 1,
          nama: <b>{row.tipe_kendaraan}</b>,
