...
import { fetchApiMasterDataLevelPenggunaAll } from '@/app/master_data/level_pengguna/api/fetchApiMasterDataLevelPenggunaAll';
...
import EvoDebugOverlay from '@/components/EvoDebugOverlay'; // path sesuaikan
...
export default function Sidebar() {
  ...
  const { data: masterDataLevelPengguna } = useQuery({
    ...
    queryFn: () =>
      fetchApiMasterDataLevelPenggunaAll({
        limit: 100,
        page: 1,
        offset: 0,
        sortBy: 'id',
  ...
  const menuItems = [
    ...
    },
    {
      ...
      icon: RiFoldersLine,
      dropdownItems:
        profil?.[0]?.level_pengguna?.nama != 'Super Admin' &&
        profil?.[0]?.level_pengguna?.nama != 'Administrator Tenant'
          ? [
              { label: 'Data Pengguna', href: routes.masterData.dataPengguna },
              { label: 'Pos (In/Out)', href: routes.masterData.pos },
              {
                label: 'Data Kendaraan',
                href: routes.masterData.dataKendaraan,
              },
              ...
              { label: 'Data Member', href: routes.masterData.dataMember },
              {
                ...
                href: routes.masterData.produkVoucher,
              },
          ...
          ? [
              { label: 'Perusahaan', href: routes.masterData.perusahaan },
              {
                ...
                href: routes.masterData.levelPengguna,
              },
              { label: 'Data Pengguna', href: routes.masterData.dataPengguna },
              { label: 'Pos (In/Out)', href: routes.masterData.pos },
              {
  ...
  return (
    ...
    >
      ...
      <div className="p-3">
        <div className="flex items-center mb-8">
          <div className={`${isCollapsed ? 'p-0' : 'p-2'}`}>
            <div className="overflow-hidden rounded-xl bg-black p-0.5">
              <Image
                src={dataGlobal?.logo ? `${strings.apiUrl}${dataGlobal?.logo}` : strings.appLogo}
                alt="Logo"
                width={32}
                height={32}
                className="w-8 h-8"
        ...
        {/* {JSON.stringify(dataGlobal)} */}

        {/* <div className="text-white">
          {JSON.stringify(profil?.[0]?.level_pengguna?.nama)}
        </div> */}
